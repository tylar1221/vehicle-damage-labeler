...
st.session_state.setdefault("current_name", "")
st.session_state.setdefault("current_side", "none")
st.session_state.setdefault("filter_mode", "All Images")
...
with tab1:
    ...
    filter_changed = filter_mode != st.session_state.filter_mode
    ...
    with col2:
        ...
        for idx, (side_key, side_label) in enumerate(sides.items()):
            with side_cols[idx]:
                if st.button(
                    side_label,
                    key=f"side_{side_key}_{st.session_state.current_name}",
                    use_container_width=True,
                    type="primary"
                    if st.session_state.current_side == side_key
                    else "secondary",
                ):
                    st.session_state.current_side = side_key
                    st.rerun()
