...
with tab1:
    filter_mode = st.selectbox(
        "📂 Show Images",
        ["All Images", "Only Unlabeled", "Only Labeled"]
    )
    ...
    elif filter_mode == "Only Labeled":
        st.session_state.images = [
            img for img in st.session_state.all_images
            if img["name"] in labeled_names
        ]
    else:
        st.session_state.images = st.session_state.all_images

    if not st.session_state.images:
        st.success("🎉 No images in this category")
        st.stop()
    ...
    with col1:
        ...
        st.caption(f"{st.session_state.index + 1} / {len(st.session_state.images)}")
    ...
    with col2:
        ...
        with b3:
            if st.button("➡️ Next", disabled=st.session_state.index == len(st.session_state.images) - 1):
                st.session_state.index += 1
                st.rerun()
...
with tab2:
    ...
    if not df.empty:
        ...
        with c2:
            st.metric("Images", len(st.session_state.all_images))
        with c3:
            st.metric(
                "Progress",
